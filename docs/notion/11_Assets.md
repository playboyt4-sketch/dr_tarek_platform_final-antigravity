...
- **1.1** (2026-08-25): Section 4.2 updated for the ratified dual storage-provider architecture (FINAL_DECISIONS §15): per-resource `storage_provider` ("bunny" | "firebase"), `system_settings.default_storage_provider` default, image attachment types (jpg/jpeg/png) added as an approved attachment option. Videos remain Bunny-only.
...
**Dual storage providers (NEW — FINAL_DECISIONS §15, approved):** PDFs, attachments and thumbnails may live on **Bunny Storage (zone `dr-tarek-resources`)** or **Firebase Storage**, chosen per file at upload time via a selector that defaults to `system_settings.default_storage_provider`. Each `lecture_resources` document records its own `storage_provider`; delivery callables dispatch to the correct backend automatically and students never know or care which provider served a file. Videos remain Bunny-only (Section 4.3). Full architecture: 06 Firebase Architecture Sections 5.3–5.4 (v1.2).

**Attachment types (approved):** zip/doc/docx/xls/xlsx/ppt/pptx/txt PLUS jpg/jpeg/png as an additional attachment option — separate from the video thumbnail upload action.
